            ...
            <div class="md-card-content large-padding">
                <div class="uk-article">
                    ...
                    <p class="uk-article-meta">
                        Ditulias oleh {{ ucwords(strtolower($article->user->name)) }} pada {{ \Carbon\Carbon::parse($article->created_at)->formatLocalized('%d %B %Y') }}
                        <br>Kategori <a href="javascript:;">{{ $article->category->name }}</a>
                    </p>
                    @if (!empty($article->thumbnail_file))
                    <img src="{{ URL::to('/thumbnails/'.$article->thumbnail_file) }}" alt="thumbnail">
                    @endif
